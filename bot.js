...
function buildEmbed(data, title = "🚑 New Application", color = 0x3b82f6) {
  const mcqText = MCQ_KEYS.map(
    (k) => `${k.toUpperCase()}: ${safe(data.mcq?.[k], 50)}`
  ).join(" | ");
  ...
  return new EmbedBuilder()
    ...
    .setTitle(title)
    .addFields(
      { name: "👤 RP Name", value: safe(data.fullName, 100), inline: true },
      { name: "💬 Discord ID", value: safe(data.discordUser, 100), inline: true },
      { name: "🧠 Score", value: `${safe(data.mcqScore, 10)}/5`, inline: true },

      { name: "📛 Ref", value: safe(data.ref, 50), inline: true },
      { name: "📌 Status", value: safe(data.status, 30), inline: true },

      {
        name: "📝 MCQ",
        value: safe(mcqText, 1000),
        inline: false,
      },

      {
        name: "💬 Explain Medical RP and why EMS must respect it",
        value: safe(data.open?.q6, 1000),
        inline: false,
      },
      {
        name: "💬 If a death is declared and there is video evidence, what do you do?",
        value: safe(data.open?.q7, 1000),
        inline: false,
      },
      {
        name: "💬 How do you deal with a patient who insults you or breaks RP rules?",
        value: safe(data.open?.q8, 1000),
        inline: false,
      },
      {
        name: "💬 Car crash: one conscious and one unconscious — who do you treat first and why?",
        value: safe(data.open?.q9, 1000),
        inline: false,
      },
      {
        name: "💬 You treated a patient but they ran away and started fighting immediately — what do you do?",
        value: safe(data.open?.q10, 1000),
        inline: false,
      }
    )
    .setFooter({ text: `REF: ${safe(data.ref, 50)}` })
    .setTimestamp();
}
...
function buildResultEmbed(data, action, ref) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: "Applicant", value: safe(data.fullName, 100), inline: true },
      { name: "Discord ID", value: safe(data.discordUser, 100), inline: true },
      { name: "Score", value: `${safe(data.mcqScore, 10)}/5`, inline: true }
    )
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  await interaction.editReply({
    embeds: [buildEmbed(data, title, color)],
